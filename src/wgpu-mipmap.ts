import shaderSource from "../src/mipmap.wgsl?raw";
 ...
 */
export const wgpuGenerateTextureMipmap = (device: GPUDevice, texture: GPUTexture) => {
...
export class WGPUMipmapGenerator {
     ...
     */
    public generateMipmaps(source: HTMLImageElement | GPUTexture | ArrayBuffer, width: number = 0, height: number = 0): GPUTexture {
        ...
        if (source instanceof ArrayBuffer) {
            if (width <= 0 || height <= 0) {
                throw new Error("Width and height must be passed if imageOrBuffer is a buffer.");
            }
        }
     ...
     */
    public async generateMipmapsAndCreateImages(source: HTMLImageElement | GPUTexture | ArrayBuffer, width: number = 0, height: number = 0): Promise<TextureImagesResult> {
    ...
    private _initPipeline() {
        ...
        this._pipelineLayout = this._device.createPipelineLayout(pipelineLayoutDesc);
    ...
    private _imageToBuffer(image: HTMLImageElement): ArrayBuffer {
        ...
        const canvasBuffer = context!.getImageData(0, 0, image.width, image.height).data;
     ...
     */
    private _initTexture(imageOrBuffer: HTMLImageElement | ArrayBuffer, width: number = 0, height: number = 0) {
    ...
    private async _computePass() {
        ...
        for (let nextMipLevel = 1; nextMipLevel < this._textureMipSizes.length; nextMipLevel++) {
            ...
            const workgroupCountX = (size.width + workgroupSizePerDimension - 1) / workgroupSizePerDimension;
            const workgroupCountY = (size.height! + workgroupSizePerDimension - 1) / workgroupSizePerDimension;

            computePass.dispatchWorkgroups(Math.floor(workgroupCountX), Math.floor(workgroupCountY), 1);
        }
